...
check_result(
  ...
  pass_if(~ identical(.result, 4), "Custom message for value 4."),
  pass_if(~ identical(.result %% 2, 0), "Any even number will work.")
)
```
````
...
`gradethis` can test against for the final returned value.
This grading approach does not inspect the code.
It only inspects the final result.

Writers can include as many testing functions as they would like.
The test functions should accept a single result value to test against.
If any test fails, it should throw an error.
This error message will be returned back to the user by default.

See `?test_result` for more information.
...
```{r test_result-check}
test_result(
  pass_if(checkmate::expect_function, args = c("x")),
  function(your_answer) {
    checkmate::expect_function(your_answer, args = c("x"))
  },
  fail_if(~ !checkmate::expect_number(.result(1)), "Make sure your function returns a number"),
  function(your_answer) {
    testthat::expect_equal(your_answer(0), NaN)
  },
  function(your_answer) {
